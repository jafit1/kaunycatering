...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <h3 className="text-base font-extrabold text-ink mb-5 pb-3 border-b border-hairline tracking-tight">{children}</h3>
}
  ...
  return (
    <div className="min-h-screen bg-surface-1 flex flex-col md:flex-row">
      ...
      <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 transition-all duration-300 pointer-events-none ${toast.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
        <div className={`flex items-center gap-3 px-4 py-3 md:px-5 md:py-3.5 rounded-2xl shadow-pop ${toast.type === 'success' ? 'bg-brand-950 text-white' : 'bg-red-600 text-white'}`}>
          {toast.type === 'success' ? (
            <div className="w-6 h-6 flex-shrink-0 bg-accent text-brand-950 rounded flex items-center justify-center">
            ...
            <div className="w-6 h-6 flex-shrink-0 bg-white/20 rounded flex items-center justify-center text-white">
      ...
      <aside className="w-full md:w-64 bg-white border-b md:border-b-0 md:border-r border-hairline flex-shrink-0 flex flex-col sticky top-0 md:h-screen z-20 shadow-aws-elevation-1 md:shadow-none">
        <div className="p-4 md:p-6 border-b border-hairline bg-white flex items-center gap-3">
          <span className="grid place-items-center h-10 w-10 rounded-xl bg-brand-800 text-accent shrink-0">
          ...
          <div className="leading-tight">
            <h2 className="text-base font-extrabold text-brand-900">Panel Admin</h2>
            <div className="text-xs text-ink-faded font-medium">{storeName}</div>
          </div>
        </div>
        <div className="flex flex-row md:flex-col gap-1 p-2 md:p-4 overflow-x-auto no-scrollbar flex-1 bg-white">
          <button
            className={`flex items-center gap-3 px-4 py-3 rounded-2xl font-semibold text-sm whitespace-nowrap transition-all duration-300 flex-1 md:flex-none ${activeTab === 'katalog' ? 'bg-brand-800 text-white shadow-soft' : 'text-ink-body hover:bg-brand-50 hover:text-brand-800'}`}
          ...
          <button
            className={`flex items-center gap-3 px-4 py-3 rounded-2xl font-semibold text-sm whitespace-nowrap transition-all duration-300 flex-1 md:flex-none ${activeTab === 'kategori' ? 'bg-brand-800 text-white shadow-soft' : 'text-ink-body hover:bg-brand-50 hover:text-brand-800'}`}
          ...
          <button
            className={`flex items-center gap-3 px-4 py-3 rounded-2xl font-semibold text-sm whitespace-nowrap transition-all duration-300 flex-1 md:flex-none ${activeTab === 'pengaturan' ? 'bg-brand-800 text-white shadow-soft' : 'text-ink-body hover:bg-brand-50 hover:text-brand-800'}`}
      ...
      {/* Main Content */}
      <main className="flex-1 p-4 md:p-10 overflow-y-auto w-full md:max-w-5xl">

        {/* ─── TAB: KATALOG ─── */}
        {activeTab === 'katalog' && (
          <div>
            <div className="bg-white rounded-3xl shadow-soft border border-hairline/70 p-5 md:p-7 animate-fade-up mb-6">
              ...
              <form onSubmit={handleAddProduct}>
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-4">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-3 border border-hairline rounded-2xl bg-white max-h-48 overflow-y-auto">
                      {categories.map(c => (
                ...
                {/* Variants Section */}
                <div className="mb-4 p-4 border border-hairline rounded-2xl bg-surface-1">
                      ...
                      <div className="flex flex-col gap-2">
                        {variants.map((v, i) => (
                          <div key={i} className="flex gap-2">
                            <input className="form-input h-11 flex-1" placeholder="Nama varian (Misal: Besar)" value={v.name} onChange={e => {
                              const next = [...variants]; next[i].name = e.target.value; setVariants(next)
                            }} required />
                            {variantType === 'DIFFERENT_PRICE' && (
                              <input type="number" className="form-input h-11 w-1/3" placeholder="Harga (Rp)" value={v.price} onChange={e => {
                                const next = [...variants]; next[i].price = e.target.value; setVariants(next)
                              }} required />
                            )}
                            <button type="button" onClick={() => setVariants(variants.filter((_, idx) => idx !== i))} className="px-3 bg-red-50 text-red-600 hover:bg-red-100 rounded-2xl font-bold transition-colors">&times;</button>
                          </div>
                        ))}
                      </div>
                      <button type="button" onClick={() => setVariants([...variants, {name: '', price: ''}])} className="mt-3 px-4 py-2 bg-white border border-hairline hover:bg-surface-1 text-ink rounded-2xl text-sm font-bold transition-colors">+ Tambah Varian</button>
                    </div>
                  )}
                </div>
                <div className="mb-6">
                  ...
                  <div className="flex gap-2">
                    <input name="imageUrl" value={newMenuImageUrl} onChange={e => setNewMenuImageUrl(e.target.value)} className="form-input flex-1" placeholder="https://..." />
                    <button type="button" onClick={handleSearchImages} disabled={isSearching} className="btn btn-accent h-12 px-5 whitespace-nowrap">
                      {isSearching ? 'Mencari...' : 'Cari Gambar'}
                    </button>
                  </div>
                  {searchImagesResult.length > 0 && (
                    <div className="mt-3 p-3 bg-surface-1 rounded-2xl border border-hairline">
                      ...
                      <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar snap-x">
                        {searchImagesResult.map((img, i) => (
                          <img 
                            ...
                            className={`w-20 h-20 object-cover rounded-2xl cursor-pointer snap-start flex-shrink-0 transition-all ${newMenuImageUrl === img ? 'ring-2 ring-primary opacity-100' : 'opacity-60 hover:opacity-100 border border-hairline'}`}
                          />
                        ))}
                      </div>
                    </div>
                ...
                <div className="flex gap-3">
                  <button type="submit" className="flex-1 btn h-12">{editingProductId ? 'Simpan Perubahan' : 'Simpan Menu'}</button>
                  {editingProductId && <button type="button" onClick={resetForm} className="flex-1 btn btn-outline h-12">Batal Edit</button>}
                </div>
              </form>
            </div>

            <div className="bg-white rounded-3xl shadow-soft border border-hairline/70 p-5 md:p-7 animate-fade-up mb-8">
              <SectionTitle>Daftar Menu ({products.length} item)</SectionTitle>
              {products.length === 0 ? (
                <div className="text-center p-8 text-ink-faded bg-surface-1 rounded-2xl border border-hairline">
                  Belum ada menu. Tambahkan menu pertama di atas.
                </div>
              ) : (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                  {products.map(p => (
                    <div key={p.id} className="bg-surface-1 rounded-xl p-2.5 flex flex-col transition-all duration-500 ease-smooth hover:bg-white hover:shadow-lift hover:-translate-y-1 group">
                      <div className="w-full aspect-square md:aspect-[4/3] bg-white rounded-2xl relative overflow-hidden">
                        <img src={p.imageUrl || 'https://via.placeholder.com/300?text=No+Image'} alt={p.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                      </div>
                      <div className="p-3 md:p-4 flex flex-col flex-1">
                        <div className="font-bold text-ink text-sm md:text-base line-clamp-2 mb-1">{p.name}</div>
                        <div className="font-extrabold text-ink text-sm md:text-base mb-1">Rp {p.price.toLocaleString('id-ID')}</div>
                        ...
                        <div className="mt-auto flex gap-2">
                          <button
                            ...
                            className="flex-1 bg-white hover:bg-brand-800 hover:text-white text-ink-body border border-hairline py-2 rounded-lg font-semibold text-xs md:text-sm transition-colors"
                          >
                            Edit
                          </button>
                          <button
                            onClick={() => setItemToDelete({ type: 'product', id: p.id, name: p.name })}
                            className="flex-1 bg-red-50 hover:bg-red-100 text-red-600 border border-red-100 py-2 rounded-lg font-semibold text-xs md:text-sm transition-colors"
                          >
                            Hapus
                          </button>
                        </div>
          ...
          <div>
            <div className="bg-white rounded-3xl shadow-soft border border-hairline/70 p-5 md:p-7 animate-fade-up mb-6">
              ...
              <form onSubmit={handleAddCategory}>
                ...
                <button type="submit" className="btn h-12 px-6">Simpan Kategori</button>
              </form>
            </div>
            ...
            <div className="bg-white rounded-3xl shadow-soft border border-hairline/70 p-5 md:p-7 animate-fade-up mb-8">
              ...
              <div className="flex flex-col gap-3">
                ...
                {categories.map(c => (
                  <div key={c.id} className="flex justify-between items-center p-4 border border-hairline rounded-2xl bg-surface-1 hover:border-hairline transition-colors">
                    <div className="font-bold text-ink flex items-center gap-3">
                      <div className="w-8 h-8 rounded-xl bg-white border border-hairline flex items-center justify-center text-primary">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-xl font-bold text-sm transition-colors"
                    >
                      Hapus
                    </button>
                  </div>
      ...
      {itemToDelete && (
        <div className="fixed inset-0 bg-brand-950/55 backdrop-blur-[6px] z-50 animate-fade-up flex items-center justify-center p-4" onClick={() => setItemToDelete(null)}>
          <div className="bg-white rounded-2xl shadow-aws-elevation-1 w-full max-w-sm p-6 text-center shadow-pop" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              ...
              <button 
                ...
                className="btn flex-1 h-12 bg-red-600 hover:bg-red-700"
              >
                Ya, Hapus
              </button>
            </div>
